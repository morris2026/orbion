...
```
platform/memory.md                          → 平台级（所有项目、所有Agent）
...
加载顺序：平台 → 项目 → Agent → 任务，后加载覆盖前面的设置。
...
- **右栏：执行** — 三 tab 面板：计划（按审批状态分组）、产出（含 react-diff-viewer diff 预览、审批操作）、Agent（运行状态卡片）
...
| [前端入口设计](1.7-mvp-ui-entry-design.md) | 三栏布局增强、Dialog、斜杠命令、产出操作 | 10 |
