...
### MVP-2.4 EventPayload schema结构与字段级验证
...
#### MVP-2.4.10 UserRegistered payload
...
### MVP-3.1 publish后subscribe的handler收到payload
...
### MVP-5.8 MemberAdded → project_members投影
...
### MVP-6.4 load_projections_impl按注册表动态加载实现类
...
### MVP-7.1 用户注册成功（非首个用户，pending状态）
...
### MVP-7.2 第一个用户注册自动审批（active状态+JWT+is_admin）
...
### MVP-12.6 Agent状态转换idle→running→idle

- **类型**：UT
- **主体流程**：dispatch→Agent开始执行→产出完成→回到idle
- **检查项**：状态转换序列正确

### MVP-12.7 Agent状态转换idle→running→error
